package com.autismindd.dao;

import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteStatement;

import de.greenrobot.dao.AbstractDao;
import de.greenrobot.dao.Property;
import de.greenrobot.dao.internal.DaoConfig;

import com.autismindd.dao.FirstLayer;

// THIS CODE IS GENERATED BY greenDAO, DO NOT EDIT.
/** 
 * DAO for table "FIRST_LAYER".
*/
public class FirstLayerDao extends AbstractDao<FirstLayer, Long> {

    public static final String TABLENAME = "FIRST_LAYER";

    /**
     * Properties of entity FirstLayer.<br/>
     * Can be used for QueryBuilder and for referencing column names.
    */
    public static class Properties {
        public final static Property Id = new Property(0, Long.class, "id", true, "_id");
        public final static Property Name = new Property(1, String.class, "name", false, "NAME");
        public final static Property FileName = new Property(2, String.class, "fileName", false, "FILE_NAME");
        public final static Property ImgUrl = new Property(3, String.class, "imgUrl", false, "IMG_URL");
        public final static Property FirstLayerTaskID = new Property(4, int.class, "firstLayerTaskID", false, "FIRST_LAYER_TASK_ID");
        public final static Property Locked = new Property(5, boolean.class, "locked", false, "LOCKED");
        public final static Property State = new Property(6, boolean.class, "state", false, "STATE");
        public final static Property CreatedAt = new Property(7, java.util.Date.class, "createdAt", false, "CREATED_AT");
    };


    public FirstLayerDao(DaoConfig config) {
        super(config);
    }
    
    public FirstLayerDao(DaoConfig config, DaoSession daoSession) {
        super(config, daoSession);
    }

    /** Creates the underlying database table. */
    public static void createTable(SQLiteDatabase db, boolean ifNotExists) {
        String constraint = ifNotExists? "IF NOT EXISTS ": "";
        db.execSQL("CREATE TABLE " + constraint + "\"FIRST_LAYER\" (" + //
                "\"_id\" INTEGER PRIMARY KEY AUTOINCREMENT ," + // 0: id
                "\"NAME\" TEXT NOT NULL ," + // 1: name
                "\"FILE_NAME\" TEXT NOT NULL ," + // 2: fileName
                "\"IMG_URL\" TEXT NOT NULL ," + // 3: imgUrl
                "\"FIRST_LAYER_TASK_ID\" INTEGER NOT NULL ," + // 4: firstLayerTaskID
                "\"LOCKED\" INTEGER NOT NULL ," + // 5: locked
                "\"STATE\" INTEGER NOT NULL ," + // 6: state
                "\"CREATED_AT\" INTEGER);"); // 7: createdAt
    }

    /** Drops the underlying database table. */
    public static void dropTable(SQLiteDatabase db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "\"FIRST_LAYER\"";
        db.execSQL(sql);
    }

    /** @inheritdoc */
    @Override
    protected void bindValues(SQLiteStatement stmt, FirstLayer entity) {
        stmt.clearBindings();
 
        Long id = entity.getId();
        if (id != null) {
            stmt.bindLong(1, id);
        }
        stmt.bindString(2, entity.getName());
        stmt.bindString(3, entity.getFileName());
        stmt.bindString(4, entity.getImgUrl());
        stmt.bindLong(5, entity.getFirstLayerTaskID());
        stmt.bindLong(6, entity.getLocked() ? 1L: 0L);
        stmt.bindLong(7, entity.getState() ? 1L: 0L);
 
        java.util.Date createdAt = entity.getCreatedAt();
        if (createdAt != null) {
            stmt.bindLong(8, createdAt.getTime());
        }
    }

    /** @inheritdoc */
    @Override
    public Long readKey(Cursor cursor, int offset) {
        return cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0);
    }    

    /** @inheritdoc */
    @Override
    public FirstLayer readEntity(Cursor cursor, int offset) {
        FirstLayer entity = new FirstLayer( //
            cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0), // id
            cursor.getString(offset + 1), // name
            cursor.getString(offset + 2), // fileName
            cursor.getString(offset + 3), // imgUrl
            cursor.getInt(offset + 4), // firstLayerTaskID
            cursor.getShort(offset + 5) != 0, // locked
            cursor.getShort(offset + 6) != 0, // state
            cursor.isNull(offset + 7) ? null : new java.util.Date(cursor.getLong(offset + 7)) // createdAt
        );
        return entity;
    }
     
    /** @inheritdoc */
    @Override
    public void readEntity(Cursor cursor, FirstLayer entity, int offset) {
        entity.setId(cursor.isNull(offset + 0) ? null : cursor.getLong(offset + 0));
        entity.setName(cursor.getString(offset + 1));
        entity.setFileName(cursor.getString(offset + 2));
        entity.setImgUrl(cursor.getString(offset + 3));
        entity.setFirstLayerTaskID(cursor.getInt(offset + 4));
        entity.setLocked(cursor.getShort(offset + 5) != 0);
        entity.setState(cursor.getShort(offset + 6) != 0);
        entity.setCreatedAt(cursor.isNull(offset + 7) ? null : new java.util.Date(cursor.getLong(offset + 7)));
     }
    
    /** @inheritdoc */
    @Override
    protected Long updateKeyAfterInsert(FirstLayer entity, long rowId) {
        entity.setId(rowId);
        return rowId;
    }
    
    /** @inheritdoc */
    @Override
    public Long getKey(FirstLayer entity) {
        if(entity != null) {
            return entity.getId();
        } else {
            return null;
        }
    }

    /** @inheritdoc */
    @Override    
    protected boolean isEntityUpdateable() {
        return true;
    }
    
}
