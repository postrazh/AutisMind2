...
public class FirstLayerJsonParser {
    Context context;
    JSONArray jsonArray;
    DatabaseManager databaseManager;
   public ArrayList<FirstLayer> getParserArrayFLayerList;
    ...
    public FirstLayerJsonParser(Context context, JSONArray jsonArray) {
        ...
        try {
            ...
            if (jsonArray != null)
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    firstLayer.setName(jresponse.getString(FIRST_LAYER_NAME));
                    int firstLayerId = Integer.parseInt(jresponse.getString(FIRST_LAYER_ID));
                    firstLayer.setFirstLayerTaskID(firstLayerId);
                    firstLayer.setFileName(jresponse.getString(FIRST_LAYER_FILE_NAME));
                    firstLayer.setImgUrl(jresponse.getString(FIRST_LAYER_IMG_PATH));
                    firstLayer.setState(false);
                    if (i == 0) {
                        firstLayer.setLocked(false);
                    } else
                        firstLayer.setLocked(true);
